...
import {
  ...
  BellIcon,
  ChartBarIcon,
   // New icon for Intern-specific flair
} from '@heroicons/react/24/outline';
import { Award } from 'lucide-react';
// =========================================================================
// !!! CORRECTED IMPORT PATHS !!!
// =========================================================================
// Assuming DashboardCalendar is located at app/staff/components/DashBoardCalendar.jsx
import DashboardCalendar from '../../(Staff)/staff/components/DashBoardCalendar';
// Assuming lib/api.js is located at app/lib/api.js
import { fetchInternDashboardSummary, fetchInternHolidays, fetchInternRecentActivities } from '../../../lib/api';


export default function InternDashboardPage() {
  // TODO: IMPORTANT: Replace this with the actual logged-in intern's employee ID
  const internId = 1; // Placeholder employee ID for demonstration

  const [userName, setUserName] = useState('Intern Member');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (!internId) {
          throw new Error('Intern ID not available for fetching dashboard data.');
        }

        const summaryData = await fetchInternDashboardSummary(internId);
        ...
        const holidaysData = await fetchInternHolidays();
        setHolidays(holidaysData.map(h => ({ ...h, date: new Date(h.date) })));

        const activitiesData = await fetchInternRecentActivities(internId);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300">
        <p>Loading intern dashboard data...</p>
      </div>
    );
  }
        ...
        <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
              <Award className="h-7 w-7 text-yellow-500" /> {/* Intern specific icon */}
              Welcome, {userName}!
            </h1>
